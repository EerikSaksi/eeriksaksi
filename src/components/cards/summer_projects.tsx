...
const SummerProjects: React.FC<{ alertCurrentlyVisible: () => void }> = ({ alertCurrentlyVisible }) => {
  ...
  const technologies = (
    <Grid style={{ WebkitOverflowScrolling: 'touch', overflow: 'scroll', maxWidth: '100%' }}>
    ...
    { question: 'What technologies did I use?', answer: technologies },
    {
      ...
      answer: (
        <Typography variant='body1'>
          ...
          , and only managed to get in contact with a fellow student of theirs who told me they'd try to get in touch with the owner, who then didn't respond to any further messages. This would be great as it would remove the need to manually synchronize the song, and the user could skip straight to preview. Other things include better data browsing, such as a global leaderboards for users and most popular songs, and better mobile optimization (although I don't know if people want to practice their texting speed.){' '}
        </Typography>
      ),
    },
  ];
  return (
    <CustomCardWithBackground progressiveImageProps={{ src: require('media/rick.gif'), preview: require('media/rick-tiny.jpg') } as ProgressiveImageProps} backgroundImageStyle={{ backgroundPosition: '20% 0%' }} alertCurrentlyVisible={alertCurrentlyVisible} cardStyle={{ maxWidth: '100%' }}>
      <React.Fragment>
        <Hidden xsDown>
