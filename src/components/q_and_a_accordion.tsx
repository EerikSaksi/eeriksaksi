    ...
    <React.Fragment>
      {questionAnswers.map(({ question, answer }, index) => (
        <Grid key = {index} item xs={12}>
          <Accordion style = {{ boxShadow: 'none' }} onChange={() => (index === currentlyOpenIndex ? setCurrentlyOpenIndex(-1) : setCurrentlyOpenIndex(index))} expanded={index === currentlyOpenIndex}>
